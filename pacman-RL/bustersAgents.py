...
class QLearningAgent(BustersAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        if os.path.exists("qtable.txt"):
            ...
            self.initializeQtable((width+height-6)*4)
    ...
    def computePosition(self, state):
        ...
        "*** YOUR CODE HERE ***"
        self.actions[state.data.agentStates[0].getDirection()]
        ...
        # if state.getGhostPositions()[index][0] <= PacX and state.getGhostPositions()[index][1] >= PacY:
        #     pacmanstate = realdistance[index]
        #     return pacmanstate

        # if state.getGhostPositions()[index][0] >= PacX and state.getGhostPositions()[index][1] >= PacY:
        #     pacmanstate = realdistance[index] + longestdistance
        #     return pacmanstate

        # if state.getGhostPositions()[index][0] <= PacX and state.getGhostPositions()[index][1] <= PacY:
        #     pacmanstate = realdistance[index] + (2*longestdistance)
        #     return pacmanstate

        # if state.getGhostPositions()[index][0] >= PacX and state.getGhostPositions()[index][1] <= PacY:
        #     pacmanstate = realdistance[index] + (3*longestdistance)
        #     return pacmanstate
    ...
    def update(self, state, action, nextState, reward):
        ...
        x = 0
        for i in state.data.ghostDistances:
            if i == None:
                x+=1
            elif i == 1 or i =="1":
                x+=3
        ...
        # if x==6:
        if (nextState.getScore() - state.getScore()) > 0:
    ...
    def getReward(self, state, action, nextstate):
        ...
        distancer = Distancer(state.data.layout)
        distancer1 = Distancer(nextstate.data.layout)
        ...
        for ghost in nextstate.getGhostPositions():
            realdistance1.append(distancer.getDistance(nextstate.getPacmanPosition(), ghost))
        index1 = realdistance1.index(min(z for z in realdistance1 if z is not None))

        if nextstate.getScore() - state.getScore() < 0 :
            if realdistance[index] - realdistance1[index1] > 0:
                return 1
